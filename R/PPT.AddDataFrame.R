"PPT.AddDataFrame" <-
function (ppt, df = stop("Data frame must be specified"), size = c(120,110,480,320),row.names=TRUE,col.names=TRUE){
    ...
    if (length(size) != 4) stop("size must be a vector of length 4")
    if(!all(is.finite(size))) stop("non-finite values detected in size vector")
